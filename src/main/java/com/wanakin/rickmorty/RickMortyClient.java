...
public class RickMortyClient {
    ...
    return webClient.get().uri("/character/"+ id).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(CharacterResponse.class);
    }

    public Mono<LocationResponse> findMyLocation(String id){
        log.info("Searching for location of [{}]",id);
        return webClient.get().uri("/location/"+id).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(LocationResponse.class);
    }

    public Mono<EpisodeResponse> findMyEpisode(String id){
        log.info("Searching for episode [{}]",id);
        return webClient.get().uri("/episode/"+id).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(EpisodeResponse.class);
    }
